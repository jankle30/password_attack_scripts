...
def master_send_fake_work (m,w_rank):
    comm = m[3]
    working = m[2]
    emptyList = numpy.zeros(2)
    #job number
    emptyList[0] = -1
    #pwlen
    emptyList[1] = 0
...
def printAllWorker(m):
    ...
    for i in range(4):
        if working[i] == 0:
            ...
            word = "undef"
        print "i am working nr:",i+1,word,working[i]
...
def worker_get_work(w):
    ...
    return (status.Get_count(MPI.BYTE) > 0), data
